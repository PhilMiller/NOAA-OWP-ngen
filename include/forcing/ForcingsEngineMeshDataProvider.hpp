...
struct ForcingsEngineMeshDataProvider
  : public ForcingsEngineDataProvider<double, MeshPointsSelector>
{
    static constexpr auto bad_index = static_cast<std::size_t>(-1);

    ~ForcingsEngineMeshDataProvider() override = default;
    ...
    static ForcingsEngineDataProvider* mesh_instance(
  ...
  private:
    ForcingsEngineMeshDataProvider(
        const std::string& init,
