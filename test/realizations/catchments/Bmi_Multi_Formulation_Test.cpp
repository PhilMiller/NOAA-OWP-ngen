...
private:
    ...
    inline void buildExampleConfig(const int ex_index, const int nested_count) {
        std::string outputVariablesSubConfig = (ex_index == 7) ? buildExampleOutputVariablesSubConfig(ex_index, true) : buildExampleOutputVariablesSubConfig(ex_index) + "\n";
        std::string config =
                "{\n"
                "    \"global\": {},\n"
                "    \"catchments\": {\n"
...
void Bmi_Multi_Formulation_Test::SetUp() {
    ...
    example_module_depth = {2, 2, 2, 2, 2, 2, 3, 2};
    ...
    initializeTestExample(7, "cat-27", {std::string(BMI_CPP_TYPE), std::string(BMI_FORTRAN_TYPE)}, { "OUTPUT_VAR_3","OUTPUT_VAR_3","OUTPUT_VAR_3" });
}
 ...
 */
TEST_F(Bmi_Multi_Formulation_Test, GetOutputLineForTimestep_7_a) {
    int ex_index = 7;
